...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Ghost mode check - BLOCK read receipts at socket level
        try {
            const ghostMode = await store.getSetting('global', 'stealthMode');
            if (!ghostMode || !ghostMode.enabled) {
                await handleAutoread(sock, message);
            } else {
                printLog('info', '👻 Ghost mode active - no presence update');
            }
        } catch (err) {
            await handleAutoread(sock, message);
        }
        ...
        // Check for sticker command FIRST
        if (message.message?.stickerMessage) {
            ...
            if (fileSha256) {
                ...
                if (stickers[hash]) {
                    ...
                    // Try to find command with prefix
        ...
        // Check for commands (prefixed OR prefixless)
        const usedPrefix = settings.prefixes.find(p => userMessage.startsWith(p));
        
        // Get command - this handles BOTH prefixed and prefixless
        const command = commandHandler.getCommand(userMessage, settings.prefixes);

        // If no prefix AND no prefixless command found, handle as regular message
        ...
        // No command found (invalid prefix command)
        if (!command) {
            if (isGroup) {
                await handleTagDetection(sock, chatId, message, senderId);
                await handleMentionDetection(sock, chatId, message);
